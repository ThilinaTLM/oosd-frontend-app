<template>
  <v-container class="pl-10 pr-10">
    ...
    <WriteLog
            title="Mark for Divisional Accept"
...
export default {
    ...
    methods: {
        ...
        async markForAccept(log) {
            this.loading = true
            const status = await api.complaint.mark.markForAccept(this.selectedComplaint.complaintId, log)
            if (status.code !== 200) {
                this.$notify(status.message, 'error')
                this.loading = false
                return
            }
            this.$notify("Complaint send to divisional office", 'success')
    ...
    async created() {
        this.loading = true
        const [complaintList, status] = await api.complaint.getComplaints({status: 'Approved'})
        if (status.code === 200) {
