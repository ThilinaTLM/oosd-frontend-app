...
export default {
    ...
    methods: {
        ...
        async markAsReviewed(log) {
            this.loading = true
            const status = await api.complaint.mark.markAsDivSecReviewed(this.selectedComplaint.complaintId, log)
            if (status.code !== 200) {
                this.$notify(status.message, 'error')
                this.loading = false
                return
            }
            this.$notify("Complaint Review Updated", 'success')
    ...
    async created() {
        ...
        const [complaintList, status] = await api.complaint.getComplaints({
            status: 'Awaiting Div Sec Review',
