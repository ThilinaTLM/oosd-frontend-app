...
export default {
    ...
    methods: {
        ...
        async markAsDisReviewed(log) {
            this.loading = true
            const status = await api.complaint.mark.markAsDisSecReviewed(this.selectedComplaint.complaintId, log)
            if (status.code !== 200) {
                this.$notify(status.message, 'error')
                this.loading = false
                return
            }
            this.$notify("Complaint Forward to Officer", 'success')
